...
purpose: A single presentation-slide thesis — one idea, one metric, 1–2 sentences
variables:
  - name: principle
    description: One of the 5 brand principles (pragmatism over hype, metrics over words, direct tone, respect for the reader, match the language) or a new principle
  - name: supporting-metric
    description: A concrete metric that backs the thesis (RPS, p99, cost, MTTR)
---
...
Write a thesis for a single slide in a Sergey Dmitriev presentation.

Principle: **{{principle}}**
Supporting metric: **{{supporting-metric}}**

Requirements:
- Length: 1–2 sentences. This is text that will sit large on a slide.
- One idea + one metric. No more. If a second thought shows up, push it to a separate slide.
- The metric from `{{supporting-metric}}` is mandatory. If empty, refuse to formulate the thesis and ask the user for a number.
- The phrasing must be punchy, readable in 3 seconds. No 20-word subject-verb monsters.
...
- No "inspirational" lines. A concrete claim you can verify.
- Render the metric as a number with a unit and context ("before/after" or absolute).

Output only the thesis text (no "Slide" label, no explanation).

---

## Example good output

Half the steps in a CI/CD pipeline duplicate each other. After the audit: pipeline 22 min → 7 min, release-bug MTTR 2 hours → 25 minutes.

## Anti-pattern (what to avoid)

Innovative cutting-edge technologies are reimagining the DevOps approach, delivering revolutionary scalability and synergy for your business!
